export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        model: "gpt-5.6-luna",
        instructions: "أنت ALMOGHANI AI، مساعد ذكي ودقيق. أجب بالعربية افتراضياً وبوضوح، وغيّر اللغة إذا طلب المستخدم.",
        input: message,
    ...
    return res.status(200).json({ reply: reply || "لم أتمكن من إنشاء رد." });
  } catch (error) {
